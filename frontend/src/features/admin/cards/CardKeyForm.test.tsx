...
describe("card key form result UI", () => {
  ...
  it("offers two days as a card key expiration option", async () => {
    ...
    await user.click(screen.getByRole("option", { name: "2 天" }));

    expect(container.querySelector<HTMLInputElement>('input[name="expiration"]')?.value).toBe("2d");
  });
});
