...
import styles from "../shared/public-pages.module.css";
...
export function RedeemForm() {
  ...
  const isLocked = redeem.isPending || isSuccess;
  const error = localError ?? (redeem.error ? redeem.error.message : undefined);

  useEffect(() => {
    if (!receiptHref) return;

    const timeoutId = window.setTimeout(() => {
      navigate(receiptHref, { replace: true });
    }, prefersReducedMotion ? 180 : 1450);

    return () => window.clearTimeout(timeoutId);
  }, [navigate, prefersReducedMotion, receiptHref]);
  ...
  return (
    <form className={cn(styles.formEnter, "space-y-4")} onSubmit={handleSubmit} noValidate>
      <AnimatePresence>
        {isSuccess ? (
          <motion.div
      ...
      <Button type="submit" size="lg" className={cn(styles.redeemButton, "h-12 w-full rounded-[10px]")} disabled={isLocked}>
        {isSuccess ? "兑换成功" : redeem.isPending ? "校验中" : "确认兑换"}
