...
import { getReceiptReturnDialog, type ReceiptReturnDialog, type ReceiptReturnState } from "../../lib/receiptReturn";
...
export function ReceiptReturnButton({ className, ...receipt }: ReceiptReturnButtonProps) {
  ...
  async function getCurrentReceiptState(): Promise<ReceiptReturnState> {
    ...
    try {
      const response = await fetch(`/api/public/receipt/${encodeURIComponent(receipt.token)}/status`, { cache: "no-store" });
