...
import { ReceiptReturnButton } from "../../components/public/ReceiptReturnButton";
    ...
    return (
      <PublicPage>
        <article className={cn(styles.receipt, styles.receiptCompact, "px-5 pb-6 pt-7 sm:px-7")}>
          <p className="text-sm text-[var(--muted)]">读取凭证中</p>
        </article>
      </PublicPage>
    );
  }
      ...
      <PublicPage>
        <article className={cn(styles.receipt, styles.receiptCompact, "px-5 pb-6 pt-7 sm:px-7")}>
          ...
          <div className={cn(styles.receiptContent, "pt-5")}>
            <Link to="/" className={cn(buttonVariants(), "w-full rounded-[10px] sm:w-auto")}>
              返回兑换页
            </Link>
          </div>
        </article>
      ...
      <article className={cn(styles.receipt, "px-5 pb-6 pt-7 sm:px-7")}>
        <header className="flex flex-col gap-5 border-b border-dashed border-[var(--line)] pb-5 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            ...
            <h1 className="mt-3 break-words text-2xl font-semibold tracking-tight sm:text-3xl">{data.goodsName}</h1>
            {data.kind === "FILE" && data.goodsNote ? (
              <p className="mt-3 max-w-2xl rounded-[8px] bg-[var(--primary-soft)] px-3 py-2 text-sm leading-6 text-[var(--primary)]">
                {data.goodsNote}
              </p>
            ) : null}
            <p className="mt-2 text-sm text-[var(--muted)]">{formatDateTime(data.redeemedAt)}</p>
          </div>
        ...
        <div className={cn(styles.receiptContent, "space-y-5 pt-5")}>
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-[8px] border border-[var(--line)] bg-[var(--surface-panel)] px-3 py-2.5">
              ...
              <p className="mt-1 text-sm font-semibold">{data.kind === "FILE" && data.downloaded ? "已下载" : "可领取"}</p>
            </div>
            <div className="rounded-[8px] border border-[var(--line)] bg-[var(--surface-panel)] px-3 py-2.5">
              <p className="text-[11px] font-semibold tracking-[0.12em] text-[var(--muted)]">{data.kind === "FILE" ? "数量" : "凭证"}</p>
              <p className="mt-1 text-sm font-semibold">{data.kind === "FILE" ? `${data.fileQuantity} 个文件` : "当前链接"}</p>
            </div>
          </div>
          ...
          {data.kind === "TEXT" ? (
            <section className={cn(styles.textPanel, "rounded-[10px] border border-[var(--line)] p-4")}>
              <div className="mb-3 flex items-center justify-between gap-3">
                ...
                <CopyTextButton text={data.textContent} />
              </div>
                  ...
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="rounded-full bg-[var(--primary-soft)] px-2.5 py-1 text-xs font-semibold text-[var(--primary)]">
                        共 {data.fileQuantity} 个文件
                      </span>
                    </div>
                    <p className="mt-1 text-sm leading-6 text-[var(--muted)]">
                      {data.downloaded ? "此凭证已完成一次成功下载。" : "文件已准备好，每个卡密只能成功下载一次。"}
                    </p>
                  </div>
                </div>
                <DownloadButton token={token} disabled={data.downloaded} className="rounded-[10px]" />
              </div>
              {data.downloaded ? (
        ...
        <div className="mt-6 flex justify-start">
          {data.kind === "FILE" ? (
            <ReceiptReturnButton kind="FILE" token={token} downloaded={data.downloaded} className="rounded-[10px]" />
          ) : (
